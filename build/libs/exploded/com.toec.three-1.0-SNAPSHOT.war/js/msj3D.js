...
function initXYZ() {
    ...
    var axisHelper = new THREE.AxisHelper(6666);
    /* scene.add(cube);*/         //-------
...
msj3D.prototype.initThree = function () {
    ...
    _this.renderer.domElement.addEventListener('contextmenu', _this.onRightClick, false);
    //$(_this.renderer.domElement).mouseover(_this.onMouseOver)
}
//初始化摄像机
...
//删除对象
msj3D.prototype.deleteObject = function (name) {
    var _this = msj3DObj;
    //获取名字为name的对象
    _obj = _this.commonFunc.findObject(name);
    //得到这个对象在objects这个json数组中的索引
    var index = _this.objects.indexOf(_obj);
    if (index > 0) {
        //根据索引删除这个对象,注意删除方法是splice(),而截取方法是slice(),不要混
        _this.objects.splice(index, 1);
        _this.scene.remove(_obj);
    } else alert('被删除物体名称不存在！')
    ...
    if (_obj.show == null || typeof (_obj.show) == 'undefined' || _obj.show) {
        ...
        switch (_obj.objType) {
            ...
            case 'cloneObj':
                ...
                /*_tempObj.position.x+=100;
                _tempObj.position.y+=100;
                _tempObj.position.x+=100;*/
                _tempObj.ip = "ip2";
                _this.addObject(_tempObj);
                break;
...
msj3D.prototype.commonFunc = {
    ...
    cloneObj: function (_objname, newparam) {
        ...
        */
        ...
        //克隆就是完全负责某一个对象，但位置信息是怎么确定的呢？
        var newobj = newobj = fobj.clone();
        //newojb是个json对象，如果这个{}包起来的对象里面还有子对象的话，子对象就是子json对象
        ...
        if (_this.commonFunc.hasObj(newparam.position)) {
            //以下就是确定位置信息，如果没有下面三句话，那么克隆的所有柜子都会重叠在一起，这就是更新位置信息
...
msj3D.prototype.addBtns = function (_btnobjs) {
    ...
    if (_btnobjs != null && _btnobjs.length > 0) {
        ...
        $("#" + _this.fId).after('<div id="orderbar" class="orderbar" ></div>');
        ...
        $("#toolbar").append('<input type="button" class="mybutton"  style="height: 30px;width: 40px"  value="X:" onclick="promptX()"><input id="textX" type="text" class="mytext"  value=""  />').append('<input type="button" class="mybutton" style="height: 30px;width: 40px" value="Y:" onclick="promptY()"><input id="textY" type="text"  class="mytext" value=""  />').append('<input type="button" class="mybutton" style="height: 30px;width: 40px" value="Z:" onclick="promptZ()"><input id="textZ" class="mytext" type="text"  value=""  />').append('<input type="button" class="mybutton" value="制作机柜" onclick="createBox()">').append('<br><br><input type="button" class="mybutton"  value="准备删除:" onclick="prepare_delete()"><input id="prepare_delete" type="text" style="border-radius: 15px; height: 40px;width: 150px"  value="" />').append('<input type="button" class="mybutton" value="删除:"  onclick="deleteCab()">').append('<br><input type="button" class="mybutton"  value="准备指令:" onclick="prepare_request2back()"><input id="prepare_request2back" type="text" style="border-radius: 15px; height: 40px;width: 150px"  value="" />').append('<input type="button" class="mybutton" value="传递指令"  onclick="request2back()">');
        $("#orderbar").append('<input id="btn_pcinfos" type="button" class="mybutton" style="height: 30px;"  value="主机标识:" onclick="btn_pcinfosx()"><textarea id="txt_pcinfos" cols="50" rows="10"></textarea>');
    }
}

function btn_pcinfosx() {
    var a=$('#btn_pcinfos').val();
    $.ajax({
        //请求方式
        type : "GET",
        //请求的媒体类型
...
function promptX() {
    var x = prompt("请输入X坐标", $('#textX').val());
    if (x != null)
        $('#textX').val(x);
}

function promptY() {
    var y = prompt("请输入Y坐标", $('#textY').val());
    if (y != null)
        $('#textY').val(y);
}

function promptZ() {
    var z = prompt("请输入Z坐标", $('#textZ').val());
    if (z != null)
        $('#textZ').val(z);
}

function prepare_delete() {
    var cabit = prompt("请输入要删除的机柜名称", 'cabinet');
    $('#prepare_delete').val(cabit);

}

function deleteCab() {
    cab = $.trim($('#prepare_delete').val())
    if (cab == '') {
        alert('被删除物名称不可为空')
        return;
    }
    msj3DObj.deleteObject(cab);
}

function createBox() {
    real_x = $.trim($('#textX').val());
    real_y = $.trim($('#textY').val());
    real_z = $.trim($('#textZ').val());
    if (real_x == '' || real_y == '' || real_z == '') {
        alert('XYZ不可有一项为空!');
        return;
    }
    var cab = {
        show: true,
        copyfrom: 'cabinet1_1',
        name: 'cabinet' + real_x + '_' + real_y + '_' + real_z,
        childrenname: ['cabinet' + real_x + '_' + real_y + '_' + real_z + '_shell', 'cabinet' + real_x + '_' + real_y + '_' + real_z + '_door_01'],
        uuid: '',
        objType: 'cloneObj',
        position: {x: -parseInt($('#textX').val()), y: parseInt($('#textY').val()), z: parseInt($('#textZ').val())},
        scale: {x: 1, y: 1, z: 1}
    }

    _tempObj = msjstation.commonFunc.cloneObj(cab.copyfrom, cab);
    msjstation.addObject(_tempObj);
}
...
msj3D.prototype.onRightClick = function (event) {
    //event.preventDefault();
    var _this = msj3DObj;
    //_this.raycaster.setFromCamera(_this.mouseClick, _this.camera);
    var intersects = _this.raycaster.intersectObjects(_this.objects);
    selectedObj = intersects[0].object;
    $('#btn_pcinfos').val(selectedObj.parent.ip);
    //alert('屏幕x:' + event.clientX + ',屏幕y:' + event.clientY);
    //alert('三维x:' + _this.mouseClick.x + ',三维y:' + _this.mouseClick.y + ',三维z:' + _this.mouseClick.z)
    /*for(i=0;i<intersects.length;i++){
         alert(intersects[i].object.name);
     }*/
    //alert('名称:' + selectedObj.name + ',x:' + selectedObj.position.x + ',y:' + selectedObj.position.y + ',z:' + selectedObj.position.z);
}
